...
which represents the last 4 nodes. When we move this list to the front then the remaining part of the initial list which is,
...
Node *appendLastNToFirst(Node *head, int n)
{
	...
	while(temp != NULL && count < length - n) {
		temp = temp -> next;
		count++;
	}
	Node *x = temp -> next;
